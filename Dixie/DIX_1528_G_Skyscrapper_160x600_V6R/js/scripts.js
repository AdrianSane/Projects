(function() {
    ...
    // initialize some settings
      ...
      mainTl
          ...
          .set(familyFrame, {scale:7, x:290, y:-195}) // scale family frame
          .set(endFrameBG, {scale:10, y:-1300, x:515}) // scale endframe bg
          .set(plate, {y:-400, rotation:25}) // position and rotate plate
          .set(cta, {scale:.5}) // set scale for cta
          .set(familyBigFrame, {x:-30, y:0});
    ...
    function main(){
        ...
        mainTl
          // frame 1
          .to(familyContainer, 2, {scale:.15, left:-75, right:0, top: 220, bottom: 0, ease: Sine.easeOut})
          .to(familyBigFrame, 2, {scale:.99,  x:-15, right:0, top: -8, bottom: 0, ease: Sine.easeOut, transformOrigin:"50% 64%"}, "-=2")
        //.to(familyBigFrame, 1, {opacity:0,}, "-=.5")
          .to(endFrameBG, 2.55, {scale:1, ease: Sine.easeOut, y:0, x:0}, "-=2.55") // reveal endframe bg
