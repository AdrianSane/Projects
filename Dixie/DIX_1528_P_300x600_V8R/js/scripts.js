(function() {
    ...
    // initialize some settings
      ...
      mainTl
          ...
          .set(familyFrame, {scale:5}) // scale family frame
          .set(endFrameBG, {scale:10, y:-600}) // scale endframe bg
          .set(plate, {y:-400, rotation:25}) // position and rotate plate
          .set(cta, {scale:.5}) // set scale for cta
          .set(familyBigFrame, {y:25}); // set scale for
    ...
    function main(){
        ...
        mainTl
            // frame 1
        .to(familyContainer, 2, {scale:.2, left:0, right:0, top: 150, bottom: 0, ease: Sine.easeOut}) // scale family container down
        .to(familyBigFrame, 2, {left:0, right:0, top: 10, bottom: 0, ease: Sine.easeOut}, "-=2") // scale big picture
            //.to(familyBigFrame, 2, {opacity:0}, "+=.5")
        .to(endFrameBG, 2.5, {scale:1.5, ease: Sine.easeOut, y:0}, "-=2.5") // reveal endframe bg
