(function() {
    ...
    // initialize some settings
      ...
      mainTl
          ...
          .set(familyFrame, {scale:7, left:-607, top:53}) // scale family frame
          .set(endFrameBG, {scale:10, x:-600}) // scale endframe bg
          .set(plate, {y:-400, rotation:10}) // position and rotate plate
          .set(cta, {scale:.5}) // set scale for cta
          .set(familyBigFrame, {scale:1.1, x:-78}); // set scale for
    ...
    function main(){
        ...
        mainTl
          // frame 1
          .to(familyContainer, 2, {scale:.15, left:162, right:0, top:-6, bottom: 0, ease: Sine.easeOut}) // scale family container down
          .to(familyBigFrame, 2, {scaleX:1.1, scaleY:1.1, x:-78, y:-11, ease: Sine.easeOut, transformOrigin:"50% 50%"}, "-=2") // hide outer family image
          .to(familyBigFrame, .5, {opacity:0}, "-=.5")
          .to(endFrameBG, 2, {scale:1.5, x:0, ease: Sine.easeOut}, "-=2") // reveal endframe bg

          // frame 2
          .to(txt, .5, {opacity:1, ease: Sine.easeIn}, "+=.2") // reveal txt
          .to(txt, .5, {opacity:0, ease: Sine.easeIn}, "+=1.5") // hide txt
          .to(familyFrame, .5, {y:-3000, ease: Sine.easeOut}) // slide family frame up
          ...
          .to(plate, 1, {opacity:1, rotation:0, y:-62, x:10, ease: Sine.easeIn}, "-=1") // reveal plate
