(function() {
    ...
    // initialize some settings
      ...
      mainTl
          ...
          .set(familyFrame, {scale:10, left:-2072, top:132, opacity: 1}) // scale family frame
          .set(endFrameBG, {scale:10, x:-2200}) // scale endframe bg
          .set(plate, {y:-400, rotation:10}) // position and rotate plate
          .set(cta, {scale:.5}) // set scale for cta
          .set(familyBigFrame, {scale:1.9, opacity:1, x:-114, y:-7}); // set scale for
    ...
    function main(){
        ...
        mainTl
          // frame 1
          .to(familyContainer, 2, {scale:.1, left:412, right:0, top:-19, bottom: 0, ease: Sine.easeOut}) // scale family container down
          .to(familyBigFrame, 2, {scale:2.05, left:300, right:0, top:14, bottom: 0, ease: Sine.easeOut}, "-=2") // scale big picture
