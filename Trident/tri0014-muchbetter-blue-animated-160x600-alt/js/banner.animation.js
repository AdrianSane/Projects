...
Banner.prototype.animate = function () {
  ...
  .to([this.txt1, this.container], .5, { y: -600, ease: Power4.easeOut }, "+=1").to([this.action1, this.action2], .5, { y: 0, ease: Power4.easeOut }, "-=.5")

  // reset txt1 for next transition
  .to(this.txt1, .01, { opacity: 0, top: 610, scale: .2 })
  ...
  // end frame
  .to(this.package, .75, { top: -160, ease: Back.easeInOut.config(1) }, "-=.5").to(this.txt1, 1, { ease: Elastic.easeOut.config(1, 0.5), scale: 1, opacity: 1 }, "-=.2").to(this.whiteBG, .5, { y: 250, ease: Power4.easeOut }, "-=1.5").from(this.replay, 0.2, { autoAlpha: 0, ease: Power3.easeOut });
