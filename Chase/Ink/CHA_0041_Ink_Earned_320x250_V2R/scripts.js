(function() {
    ...
    mainTl.set(bottom_white, {y: 182 });
    mainTl.set(banner, {autoAlpha:1});
    mainTl.set(card, {x: 38, y: 82, force3D:true, rotateZ:0.001});
    mainTl.set(chase_logo, {x: 69, y: 226});
    ...
    function main() {
        ...
        mainTl
            ...
            .add("start", 0.5)
            // show the card
            .from(card, 0.75, {y: 300, ease:Power3.easeOut}, "start+=0.2")
            // show the bottom white footer area
            .from(bottom_white, 0.5, {y: HEIGHT, height: 0, transformOrigin:"bottom", ease:Power3.easeOut}, "start+=0.2")
            // show the logo
            ...
            // scale the card down
            .to(card, 0.8, {scale:0.42, x:144, y:150, ease:Power3.easeOut}, "frame2+=0.2")
            .to(bottom_white, 0.4, {y: "+=34", ease: Power3.easeOut}, "frame2+=0.2")

            // move the logo left
            .to(chase_logo, 0.4, {x:19, ease:Power1.easeOut}, "frame2+=0.2")
            ...
            // scale card
            .to(card, 0.4, {scale:0.67, x:46, y:101, ease:Power3.easeOut}, "endframe+1")
            .to(bottom_white, 0.6, {y: 190, ease:Power3.easeOut}, "endframe+1")

            .to(replay, 0.3, {autoAlpha:1, ease:Power3.easeOut}, "endframe+=1.4")
            .from(replay, 0.8, {rotation:-180, transformOrigin:"6px 6px", ease:Power3.easeOut}, "endframe+=1.4")
    ...
    document.getElementById('hit').onmouseout =
      ...
      replay.onmouseover = function(event) {
        TweenMax.to(replay, 0.3, {rotation:180, transformOrigin:"6px 6px"})
      };

      replay.onmouseout = function(event) {
        TweenMax.to(replay, 0.3, {rotation:0, transformOrigin:"6px 6px"})
      };
